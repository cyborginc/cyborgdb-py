...
class TestClientListIndexes(unittest.TestCase):
    ...
    def test_unauthorized_raises_authentication_error(self):
        ...
        self.client.api.list_indexes_v1_indexes_list_get.side_effect = UnauthorizedException(status=401)
    ...
    def test_forbidden_raises_authentication_error(self):
        ...
        self.client.api.list_indexes_v1_indexes_list_get.side_effect = ForbiddenException(status=403)
        with self.assertRaises(AuthenticationError) as ctx:
            self.client.list_indexes()
    ...
    def test_service_exception_503_raises_service_unavailable(self):
        ...
        self.client.api.list_indexes_v1_indexes_list_get.side_effect = ServiceException(status=503)
    ...
    def test_service_exception_504_raises_service_unavailable(self):
        ...
        self.client.api.list_indexes_v1_indexes_list_get.side_effect = ServiceException(status=504)
    ...
    def test_service_exception_500_raises_service_unavailable(self):
        ...
        self.client.api.list_indexes_v1_indexes_list_get.side_effect = ServiceException(status=500)
        with self.assertRaises(ServiceUnavailableError):
            self.client.list_indexes()

    def test_timeout_raises_connection_timeout_error(self):
        """Case 6: urllib3 TimeoutError -> ConnectionTimeoutError."""
        self.client.api.list_indexes_v1_indexes_list_get.side_effect = urllib3.exceptions.TimeoutError()
    ...
    def test_bad_request_raises_value_error(self):
        ...
        self.client.api.list_indexes_v1_indexes_list_get.side_effect = BadRequestException(status=400)
    ...
    def test_not_found_raises_value_error(self):
        ...
        self.client.api.list_indexes_v1_indexes_list_get.side_effect = NotFoundException(status=404)
...
class TestEncryptedIndexQuery(unittest.TestCase):
    ...
    def test_unauthorized_raises_authentication_error(self):
        self.idx._api.query_vectors_v1_vectors_query_post_without_preload_content.side_effect = UnauthorizedException(status=401)
        with self.assertRaises(AuthenticationError) as ctx:
            self._call_query()
        self.assertIsInstance(ctx.exception.__cause__, UnauthorizedException)

    def test_service_exception_raises_service_unavailable(self):
        self.idx._api.query_vectors_v1_vectors_query_post_without_preload_content.side_effect = ServiceException(status=503)
        with self.assertRaises(ServiceUnavailableError):
            self._call_query()
    ...
    def test_new_connection_error_raises_service_unavailable(self):
        conn = MagicMock()
        self.idx._api.query_vectors_v1_vectors_query_post_without_preload_content.side_effect = (
            urllib3.exceptions.NewConnectionError(conn, "refused")
        )
        with self.assertRaises(ServiceUnavailableError):
            self._call_query()
...
class TestEncryptedIndexUpsert(unittest.TestCase):
    ...
    def test_unauthorized_raises_authentication_error(self):
        self.idx._api.upsert_vectors_v1_vectors_upsert_post.side_effect = UnauthorizedException(status=401)
        with self.assertRaises(AuthenticationError):
            self._call_upsert()

    def test_service_exception_raises_service_unavailable(self):
        self.idx._api.upsert_vectors_v1_vectors_upsert_post.side_effect = ServiceException(status=500)
        with self.assertRaises(ServiceUnavailableError):
            self._call_upsert()

    def test_timeout_raises_connection_timeout(self):
        self.idx._api.upsert_vectors_v1_vectors_upsert_post.side_effect = urllib3.exceptions.TimeoutError()
