    ...
    def test_ivfpq_parameter_validation(self):
        ...
        try:
            ...
            try:
                ...
                self.fail("Expected validation error for pq_dim=0, but index creation succeeded")
            except Exception as e:
                # Expected - should reject invalid pq_dim
                self.assertIn("pq_dim", str(e).lower(), f"Error should mention pq_dim: {e}")
...
class TestErrorHandling(unittest.TestCase):
    ...
    def test_invalid_api_key(self):
        ...
        try:
            ...
            auth_related = any(keyword in error_str for keyword in 
                             ['auth', 'key', 'unauthorized', '401', 'forbidden', '403'])
            if auth_related:
                self.assertTrue(True, f"Invalid API key properly rejected: {e}")
            else:
                self.skipTest(f"Got non-authentication error, possibly network issue: {e}")
    ...
    def test_malformed_requests(self):
        ...
        try:
            ...
            with self.assertRaises(Exception):  # Could be ValueError or other exception type
    ...
    def test_invalid_vector_dimensions(self):
        ...
        try:
            ...
            with self.assertRaises(Exception):
                invalid_vector = np.random.rand(64).astype(np.float32)  # Wrong dimension
                index.upsert([{
                    "id": "test",
    ...
    def test_mismatched_parameter_lengths(self):
        ...
        try:
            ...
            for expected_id in expected_ids:
                self.assertIn(expected_id, stored_ids, f"Item {expected_id} should be stored")
                
        except Exception as e:
            # If it fails, verify it's a validation error, not a network error
            error_str = str(e).lower()
            validation_related = any(keyword in error_str for keyword in 
                                   ['validation', 'length', 'mismatch', 'parameter'])
            self.assertTrue(validation_related, 
