...
def make_index(client, prefix="conc"):
    """Create a uniquely-named IVFFlat index and return (index, name, key)."""
...
class TestPerformanceUnderConcurrentLoad(unittest.TestCase):
    """
    Measure throughput degradation under concurrency. Single-thread baseline
    vs 10-thread concurrent upserts — ensures contention doesn't cause
    catastrophic slowdown (threshold: 5x, generous for network variability).
    """
    ...
    def test_concurrent_throughput_not_catastrophically_worse(self):
        """
        Baseline: 1 thread upserts 5 batches of 50 vectors.
        Concurrent: 10 threads each upsert 5 batches of 50 vectors (same work per thread).
        If fully serialized, wall-clock would be ~10x baseline. Threshold is 8x
        to leave room for server contention while still catching deadlocks
        ...
        for i in range(batch_count):
            upsert_batch(self.index, f"baseline_{i}", count=NUM_VECTORS)
        single_elapsed = time.monotonic() - start

        # 10-thread concurrent — each thread does the same work as baseline
        errors = []
        lock = threading.Lock()
        ...
        def worker(thread_id):
            try:
                for i in range(batch_count):
                    upsert_batch(self.index, f"conc_{thread_id}_{i}", count=NUM_VECTORS)
            except Exception as e:
                with lock:
                    errors.append((thread_id, e))
        ...
        self.assertEqual(len(errors), 0, f"Concurrent upsert errors: {errors}")

        # If concurrency works at all, 10 threads doing the same work each
        # should finish in well under 10x the single-thread time.
        # Perfect parallelism = 1x, full serialization = 10x.
        # Threshold 8x catches serialization/deadlock while tolerating
        # server contention, GIL overhead, and CI variability.
        ratio = concurrent_elapsed / single_elapsed if single_elapsed > 0 else 0

        self.assertLess(
            ratio, 8.0,
            f"Concurrent wall-clock is {ratio:.1f}x single-thread — "
            f"near full serialization "
            f"(single: {single_elapsed:.3f}s, "
            f"concurrent: {concurrent_elapsed:.3f}s, "
            f"perfect parallelism would be ~1x, full serialization = {num_threads}x)"
        )
