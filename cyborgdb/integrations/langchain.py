...
try:
    ...
    class CyborgVectorStore(VectorStore):
        ...
        def _setup_embedding_model(self, embedding: Union[str, Embeddings, SentenceTransformer]) -> None:
        ...
        def _create_new_index(
            ...
            config = self._create_index_config(index_type, dimension, metric, index_config_params)
        ...
        def _detect_embedding_dimension(self) -> int:
            ...
            if self.embedding_model is None:
                raise RuntimeError("No embedding model provided and dimension not specified")
            ...
            if hasattr(self.embedding_model, "get_sentence_embedding_dimension"):
                ...
                return len(dummy) if isinstance(dummy, list) else np.asarray(dummy).shape[0]
            else:
                # Try encode method
        ...
        def _create_index_config(
            ...
            if index_type == "ivf":
                ...
                raise ValueError(f"Invalid index type: {index_type}. Must be 'ivf', 'ivfpq', or 'ivfflat'")
        ...
        def get_embeddings(self, texts: Union[str, List[str]]) -> np.ndarray:
            ...
            if hasattr(self.embedding_model, 'encode') and hasattr(self.embedding_model, 'get_sentence_embedding_dimension'):
                ...
                raise TypeError(f"Unsupported embedding model type: {type(self.embedding_model)}")
        ...
        def _normalize_score(self, distance: float) -> float:
            ...
            try:
                ...
                metric = config.get('metric', 'cosine') if isinstance(config, dict) else getattr(config, 'metric', 'cosine')
            except:
                metric = 'cosine'
